...
# Pattern: Attribution and showback API endpoints
...
async def get_user_spending_ranking(
    ...
    """
    User ranking by LLM spend in the period.
    Includes whether the user has an active budget override.
    """
...
async def get_service_breakdown(days: int = Query(30, ge=1, le=365)):
    """
    Spend breakdown by service and model.
    Identifies which business operation generates the most cost.
    """
    since = datetime.now(timezone.utc) - timedelta(days=days)
