...
# Pattern: Streaming LLM call with cost tracking
...
async def astream_with_tracking(
    ...
    **kwargs,
    ...
    """
    Streaming with tracking at the end of the stream.
    Usage tokens are only available in the last event.
    """
    ...
    async with self._llm.astream_events(messages, version="v2", **kwargs) as stream:
        async for event in stream:
            # Capture tokens from the final usage event
    ...
    # Log usage upon stream completion
