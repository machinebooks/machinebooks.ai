...
# gateway/app/api/v1/embeddings.py:62-75 (synthesized)
...
for raw in inputs:
    ...
    clean = PolicyService.sanitize_input_variables(raw)  # 2. sanitize vars
    clean, pii_count, pii_types = await PolicyService.scan_and_sanitize(
        clean, db, organization_id=org_id,         # 3. PII per org
    )
