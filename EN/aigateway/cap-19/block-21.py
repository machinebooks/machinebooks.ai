...
async def run(ctx) -> bool:
    ...
    # 2. Content classifier — 15 categories
    classification = await ContentClassifierService.classify(full_prompt, ...)

    # 3. Leak detection — extra credentials + behavioral reconnaissance
    leak_scan = await LeakDetectionService.scan_input(full_prompt, redis, device_id)

    # 4. Jailbreak detector v2 — NFKC scoring with device bypass
    ...
    # 5. Configurable guardrails (DB) — complements hardcoded with tunable rules
