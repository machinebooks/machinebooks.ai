...
class CloudCostMetric(Base):
    """
    Serie temporal de costes por servicio y proveedor.
    Una fila por servicio por hora.
    """
    ...
    # Índices para queries de serie temporal
    __table_args__ = (
...
class CostAnomaly(Base):
    """Registro de anomalías detectadas para trazabilidad y aprendizaje."""
    ...
    cost_usd = Column(Float, nullable=False)         # Coste actual
    expected_cost_usd = Column(Float, nullable=False) # Media histórica
    pct_deviation = Column(Float, nullable=False)     # Desviación porcentual
    ...
    was_false_positive = Column(Integer, nullable=True)  # 1=sí, 0=no
