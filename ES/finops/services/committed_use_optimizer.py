...
class CommittedUseOptimizer:
    """
    Calcula el nivel óptimo de committed use
    basándose en el histórico de consumo.
    """

    def optimize(
        self,
        historical_monthly_usd: list,   # consumo de los últimos N meses
        ...
        return {
            ...
            "conservative_committed_usd": round(p10, 2),  # mínimo riesgo
            "aggressive_committed_usd": round(p60, 2),     # mayor ahorro
            "rationale": (
                "Alta varianza: recomendación conservadora."
                if cv > 0.20 else
                "Baja varianza: se puede considerar nivel agresivo."
            ),
        }
