...
# Validación de bearer (gateway/app/services/user_api_key_service.py)
async def validate_bearer(db: AsyncSession, raw_token: str) -> Optional[UserApiKey]:
    ...
    result = await db.execute(
        select(UserApiKey).where(
            ...
            or_(
                UserApiKey.key_hash == token_hash,                    # primaria
                and_(
                    UserApiKey.secondary_key_hash == token_hash,      # secundaria
                    or_(
